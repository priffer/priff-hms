-- =====================================================
-- database/34_payroll_engine_pay_frequency_filter.sql
-- Milestone 5 Phase 2 (ต่อ): อัปเดต fn_run_payroll_period() ให้กรองพนักงานตาม pay_frequency
-- ให้ตรงกับ period_type ของงวดที่กำลังรัน (สำคัญมาก - กันไม่ให้พนักงานรายเดือนถูกคำนวณเงินเดือน
-- เต็มเดือนซ้ำถ้ามีคนไปรันงวดรายวัน/กึ่งเดือนโดยไม่ตั้งใจ)
--
-- กฎกรอง:
--   - period_type='monthly'     -> ประมวลผลเฉพาะพนักงานที่ COALESCE(pay_frequency, 'monthly') = 'monthly'
--     (พนักงานรายเดือนที่ยังไม่ตั้งค่า pay_frequency ถือว่าเป็น monthly โดยอัตโนมัติ เพราะเป็นค่าเดียว
--     ที่สมเหตุสมผลสำหรับ salary_type='monthly')
--   - period_type='semimonthly' -> ประมวลผลเฉพาะพนักงานที่ pay_frequency = 'semimonthly' เท่านั้น
--     (ไม่เดา - ถ้ายังไม่ตั้งค่าจะไม่ถูกประมวลผล ต้องให้แอดมินตั้งค่าก่อน)
--   - period_type='daily'       -> ประมวลผลเฉพาะพนักงานที่ pay_frequency = 'daily' เท่านั้น (ไม่เดาเช่นกัน)
--
-- v_periods_per_year (ใช้คำนวณภาษี annualization method) เพิ่มค่า semimonthly = 24 งวด/ปี
--
-- ต้องรัน 33_pay_frequency_and_period_autogen.sql ก่อนไฟล์นี้เสมอ
