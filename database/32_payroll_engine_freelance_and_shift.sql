-- =====================================================
-- database/32_payroll_engine_freelance_and_shift.sql
-- Milestone 5 Phase 1 (ต่อ): อัปเดต fn_run_payroll_period() ให้รองรับ
--   - employment_type='freelance': ไม่หักประกันสังคม + หัก ณ ที่จ่ายคงที่ 3% (แทนตารางขั้นบันได)
--   - ค่ากะดึก (night_shift_differential): +10% ของค่าแรง/วันฐาน สำหรับวันที่ทำงานจริงในกะที่ข้าม
--     เที่ยงคืน (employee_shift_assignments.shift_end < shift_start ณ วันนั้น)
--
-- ต้องรัน 31_employment_type_and_shift_differential.sql ก่อนไฟล์นี้เสมอ (ต้องมีคอลัมน์
-- employees.employment_type และอัตรา freelance_withholding_tax/night_shift_differential ใน
-- payroll_rates ก่อน)
--
-- ไฟล์นี้ทำ CREATE OR REPLACE FUNCTION ทับของเดิมทั้งฟังก์ชัน (ฐานจาก 27_payroll_engine.sql)
-- เพิ่มเฉพาะ 2 ส่วนที่ระบุข้างบน ตรรกะเดิมอื่นๆ (OT/ขาดงาน/เบิกล่วงหน้า/weekly OT flag) ไม่เปลี่ยน
--
...
-- SELECT * FROM public.payroll_line_details WHERE detail_type = 'night_shift' ORDER BY created_at DESC LIMIT 10;
