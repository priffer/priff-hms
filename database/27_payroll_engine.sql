-- =====================================================
-- database/27_payroll_engine.sql
-- Payroll Engine Milestone 4: fn_run_payroll_period() - เครื่องคำนวณเงินเดือนจริง
--
-- อ้างอิง docs/payroll-architecture.md และ docs/payroll-database-design.md ทั้งฉบับ
-- ยึดหลักการที่ตัดสินใจร่วมกับ user แล้ว (ask_user ใน session นี้):
--   - OT อ่านจาก attendance_ot_details ที่ materialize ไว้แล้ว (25_attendance_ot_materialize.sql)
--   - ภาษีใช้ payroll_tax_brackets ปี 2026 จริงจากกรมสรรพากร (26_payroll_rates_seed.sql)
--   - อัตรา OT/ประกันสังคม อ่านจาก payroll_rates (26_payroll_rates_seed.sql)
--   - รองรับทั้ง salary_type = 'monthly' และ 'daily'
--   - บล็อกการอนุมัติอัตโนมัติถ้าสัปดาห์ไหนใน period มี weekly_ot_flagged = true (docs §4.7)
--
-- ⚠️ หมายเหตุสำคัญ - จุดที่เป็น "ประมาณการ/แนวทางที่ implement เพิ่มเติม" เพราะเอกสารต้นฉบับ
-- ไม่ได้ระบุสูตรละเอียดระดับนี้ไว้ (ไม่ใช่ 4 gap ที่ถามผู้ใช้ไปแล้ว แต่เป็นรายละเอียดย่อยที่ต้อง
-- ตัดสินใจเพื่อให้ engine ทำงานได้จริง) - ทำเครื่องหมายไว้ชัดเจนเพื่อให้ตรวจสอบ/ปรับได้ง่าย:
--   [A] ค่าแรง/วัน ของพนักงานรายเดือน (ใช้หักขาดงาน) = hourly_rate(=monthly_salary/standard_monthly_hours)
--       * standard_working_hours (default 8) - เอกสาร §4.6 บอกแค่ "หักตาม daily_rate หรือชั่วโมงที่ขาด"
--       ไม่ได้ระบุสูตรพนักงานรายเดือนที่ไม่มี daily_rate ชัดเจน
--   [B] ภาษีหัก ณ ที่จ่ายใช้วิธี "ประมาณการรายได้ทั้งปี" (annualization method) ตามที่กรมสรรพากร
--       แนะนำจริงสำหรับพนักงานประจำ: รายได้งวดนี้ x จำนวนงวด/ปี = รายได้ประมาณการทั้งปี, หักลดหย่อน,
--       คำนวณภาษีทั้งปีตามตาราง bracket, หารกลับด้วยจำนวนงวด/ปี = ภาษีหักงวดนี้ (รายเดือน x12,
--       รายวัน x260 วันทำงาน/ปีโดยประมาณ) - เป็นวิธีมาตรฐานที่ใช้จริง แต่ยังไม่ได้ปรับ YTD ระหว่างปี
--       ถ้าเงินเดือนเปลี่ยนกลางปี (ยังไม่ implement retroactive adjustment เต็มรูปแบบ - เก็บ
--       ytd_income/ytd_tax_paid ไว้ให้ตรวจสอบได้ แต่ไม่ recalculate ย้อนหลังอัตโนมัติ)
--   [C] เงินเบิกล่วงหน้าที่หัก = เต็มจำนวนที่อนุมัติแล้วและยังไม่ถูกหัก (ไม่ผ่อนหักหลายงวด) - ถ้า
--       ยอดเบิกมากกว่า net_pay ที่เหลือ ให้หักเท่าที่ net_pay เหลือพอ (ไม่ให้ net_pay ติดลบ) ส่วนที่
--       เหลือค้างไว้งวดถัดไป (ไม่ mark deducted_in_payroll_run_id จนกว่าจะหักครบ)
--
-- ต้องรัน 24/25/26 (weekly_rest_day, materialize, rates seed) ก่อนไฟล์นี้เสมอ
-- Idempotent: CREATE OR REPLACE FUNCTION - เรียกซ้ำสำหรับ period เดียวกันจะสร้าง run ใหม่เสมอ
-- (ไม่ auto-overwrite run เก่า - ให้ admin จัดการ draft runs ซ้ำที่ไม่ต้องการเอง)
...
BEGIN
  ...
  -- สร้าง payroll_run ใหม่เสมอ (draft) - ผลลัพธ์เขียนเข้า run นี้
  INSERT INTO public.payroll_runs (period_id, company_id, run_name, run_type, status, created_by)
  VALUES (p_period_id, v_company, 'Payroll Run ' || to_char(now(), 'YYYY-MM-DD HH24:MI'), v_period.period_type, 'draft', p_created_by)
  RETURNING id INTO v_run_id;

  -- วน loop พนักงานที่ active และตั้งค่า salary_type ไว้แล้วเท่านั้น (ยังไม่ตั้งค่า = ข้าม ไม่เดา)
  FOR v_emp IN
  ...
  LOOP
    ...
    v_gross_pay := v_base_salary - v_absence_deduction + v_ot_amount;
    IF v_gross_pay < 0 THEN v_gross_pay := 0; END IF;

    -- ประกันสังคม ม.33: MIN(ฐาน x 5%, 875) ต่อเดือน (ใช้ social_security_base ถ้าตั้งไว้ ไม่งั้นใช้ base_salary)
    ...
    -- [B] ภาษีหัก ณ ที่จ่าย - annualization method
    v_periods_per_year := CASE WHEN v_period.period_type = 'monthly' THEN 12 ELSE 260 END;
    ...
    VALUES
      ...
      (v_line_id, v_company, 'social_security', 'ประกันสังคม (พนักงาน)', -v_ss_employee, 1, v_ss_emp_rate),
      (v_line_id, v_company, 'withholding_tax', 'ภาษีหัก ณ ที่จ่าย', -v_withholding_tax, 1, NULL),
...
-- =====================================================
-- SELECT public.fn_run_payroll_period('<period_id>', '<user_profile_or_auth_uid>');
-- SELECT * FROM public.payroll_runs ORDER BY created_at DESC LIMIT 5;
-- SELECT * FROM public.payroll_lines WHERE payroll_run_id = '<run_id>';
-- SELECT * FROM public.payroll_line_details pld JOIN public.payroll_lines pl ON pld.payroll_line_id = pl.id WHERE pl.payroll_run_id = '<run_id>' ORDER BY pl.emp_id, pld.detail_type;
