...
-- Idempotent cleanup: drop existing policies, triggers and functions that this migration will (re)create
-- This ensures migration is idempotent for re-runs on the same database (use with caution; backup recommended)
-- Drop policies
...
-- Drop triggers
DROP TRIGGER IF EXISTS trg_attendance_compute_weekly_ot ON public.attendance_logs;
DROP TRIGGER IF EXISTS trg_attendance_audit ON public.attendance_logs;
DROP TRIGGER IF EXISTS trg_attendance_modify_allowed ON public.attendance_logs;

-- Drop functions (if present)
DROP FUNCTION IF EXISTS public.fn_compute_iso_week_and_weekly_ot() CASCADE;
DROP FUNCTION IF EXISTS public.fn_attendance_audit() CASCADE;
DROP FUNCTION IF EXISTS public.fn_attendance_modify_allowed() CASCADE;
-- Note: helper functions (get_user_profile_id, get_user_company, get_user_role, is_active_user) will also be recreated below
DROP FUNCTION IF EXISTS public.get_user_profile_id() CASCADE;
