-- 01_payroll_migration.sql
-- SQL Migration script สำหรับการเพิ่ม schema Payroll ใน PRIFF-HMS
-- หมายเหตุสำคัญ:
...
CREATE TABLE IF NOT EXISTS public.payroll_lines (
  ...
  overtime_hours jsonb DEFAULT '{}'::jsonb,
...
-- NOTE: Policies ที่นี่เป็นตัวอย่างเชิงนโยบายพื้นฐาน ควรปรับตาม security review ก่อนรันจริง
...
CREATE POLICY ot_select ON public.attendance_ot_details
  ...
  USING (
    ...
    OR (public.get_user_role() = 'supervisor' AND company_id = public.get_user_company() AND attendance_ot_details.client_id IS NULL)
    OR (public.get_user_role() = 'payroll' AND company_id = public.get_user_company())
  );
...
BEGIN
  -- compute ISO week
  v_iso_week := extract(isodow FROM NEW.work_date) + 0; -- placeholder; use date_part('isoyear', ..) if needed
  -- better: use to_char(NEW.work_date, 'IYYY-IW') for iso year-week but store integer week number here
  ...
  SELECT COALESCE(SUM(ot_hours),0) INTO v_weekly_ot
  ...
  -- include current record's ot_hours if present (assuming NEW.ot_hours exists)
  IF NEW.ot_hours IS NOT NULL THEN
    v_weekly_ot := v_weekly_ot + NEW.ot_hours;
  END IF;
...
-- 5.5 Trigger for payroll_runs state change to lock related attendance
CREATE OR REPLACE FUNCTION public.fn_payroll_run_state_change()
RETURNS trigger
LANGUAGE plpgsql
AS $$
BEGIN
  IF TG_OP = 'UPDATE' THEN
    IF NEW.status IN ('approved','locked') AND OLD.status NOT IN ('approved','locked') THEN
      -- set attendance_locked for attendance_logs in period
      UPDATE public.attendance_logs al
      SET attendance_locked = true, locked_at = now()
      FROM public.payroll_periods pp
      WHERE pp.id = NEW.period_id AND al.company_id = NEW.company_id
        AND al.work_date >= pp.period_start AND al.work_date <= pp.period_end;
    END IF;
  END IF;
  RETURN NEW;
END;
...
CREATE TRIGGER trg_payroll_run_state_change
AFTER UPDATE ON public.payroll_runs
FOR EACH ROW
EXECUTE FUNCTION public.fn_payroll_run_state_change();
...
- ฟังก์ชัน RLS helper อ้างอิงตาราง user_profiles ที่ต้องถูก populate ก่อนใช้งานโดย client
- การ backfill ข้อมูล (populate auth_uid, company_id, iso_week, weekly_ot_hours) ควรรันเป็น batch นอก transaction เพื่อป้องกัน lock นาน
